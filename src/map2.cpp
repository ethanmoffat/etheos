...
void map_spawn_chests(void *map_void) {
  ...
  UTIL_FOREACH(map->chests, chest) {
    ...
    double spawn_random =
        static_cast<double>(map->world->config["ChestSpawnRandomization"]);
    bool spawn_random_is_pct =
        *static_cast<std::string>(map->world->config["ChestSpawnRandomization"])
             .rbegin() == '%';

    UTIL_FOREACH(spawns, slot_spawns) {
      if (!slot_spawns.empty()) {
        Map_Chest_Spawn *spawn = *std::next(
            slot_spawns.begin(), util::rand(0, slot_spawns.size() - 1));
...
#ifdef DEBUG
        Console::Dbg(
            "Spawning chest item %i (x%i) on map %i (timer %is | random %.2fs)",
            spawn->item.id, spawn->item.amount, map->id,
            static_cast<int>(spawn->time), spawn->spawn_random);
#endif // DEBUG

        double spawn_modifier = util::rand(double(0), spawn_random);
        spawn->spawn_random =
            spawn_random_is_pct ? spawn_modifier * spawn->time : spawn_modifier;
      }
    }
...
void map_close_door(void *map_close_void) {
  map_close_door_struct *close(
      static_cast<map_close_door_struct *>(map_close_void));
...
void map_evacuate(void *map_evacuate_void) {
  map_evacuate_struct *evac(
      static_cast<map_evacuate_struct *>(map_evacuate_void));

  int ticks_per_step = int(evac->map->world->config["EvacuateStep"]) /
                       int(evac->map->world->config["EvacuateTick"]);
  ...
  if (evac->step > 0) {
    ...
    UTIL_FOREACH(evac->map->characters, character) {
      if (step)
        character->ServerMsg(character->world->i18n.Format(
            "map_evacuate", (evac->step / ticks_per_step) *
                                int(evac->map->world->config["EvacuateStep"])));
...
int Map_Chest::AddItem(short item_id, int amount, int slot) {
  ...
  if (this->items.size() >= static_cast<std::size_t>(this->chestslots) ||
      amount > this->maxchest) {
    return 0;
  }
...
Map::Map(int id, World *world) {
  ...
  if (!this->chests.empty()) {
    TimeEvent *event =
        new TimeEvent(map_spawn_chests, this, 1.0, Timer::FOREVER);
...
void Map::LoadArena() {
  ...
  if (world->arenas_config[util::to_string(id) + ".enabled"]) {
    std::string spawns_str =
        world->arenas_config[util::to_string(id) + ".spawns"];
    ...
    if (spawns.size() % 4 != 0) {
      ...
      this->arena = new Arena(
          this,
          static_cast<int>(world->arenas_config[util::to_string(id) + ".time"]),
          static_cast<int>(
              world->arenas_config[util::to_string(id) + ".block"]));
  ...
  UTIL_FOREACH(update_characters, character) {
    ...
    if (!this->arena) {
      character->Warp(character->map->id, character->map->relog_x,
                      character->map->relog_y);
    }
  }
}
...
bool Map::Load() {
  ...
  for (int i = 0; i < outersize; ++i) {
    ...
    for (int ii = 0; ii < innersize; ++ii) {
      ...
      if (!this->InBounds(xloc, yloc)) {
        Console::Wrn("Tile spec on map %i is outside of map bounds (%ix%i)",
                     this->id, xloc, yloc);
        continue;
      }

      this->GetTile(xloc, yloc).tilespec =
          static_cast<Map_Tile::TileSpec>(spec);
  ...
  for (int i = 0; i < outersize; ++i) {
    ...
    for (int ii = 0; ii < innersize; ++ii) {
      ...
      newwarp.spec = static_cast<Map_Warp::WarpSpec>(
          PacketProcessor::Number(buf[6], buf[7]));

      if (!this->InBounds(xloc, yloc)) {
        Console::Wrn("Warp on map %i is outside of map bounds (%ix%i)",
                     this->id, xloc, yloc);
        continue;
      }
    ...
    for (int ii = 0; ii < amount; ++ii) {
      if (!this->InBounds(x, y)) {
        Console::Wrn(
            ...
            this->id, this->world->enf->Get(npc_id).name.c_str(), x, y);
        continue;
      }
  ...
  double spawn_random =
      static_cast<double>(this->world->config["ChestSpawnRandomization"]);
  bool spawn_random_is_pct =
      *static_cast<std::string>(this->world->config["ChestSpawnRandomization"])
           .rbegin() == '%';
  ...
  for (int i = 0; i < outersize; ++i) {
    ...
    if (amount == 0) {
      Console::Wrn("A chest spawn on map %i has an amount of 0 (#%i at %ix%i) "
                   "and will be ignored",
                   this->id, itemid, x, y);
      continue;
    }
    ...
    Console::Wrn(
        ...
        this->id, this->world->eif->Get(itemid).name.c_str(), amount, x, y);
  skip_warning:;
  }
...
void Map::Msg(NPC *from, std::string message) {
  message = util::text_cap(
      message, static_cast<int>(this->world->config["ChatMaxWidth"]) -
                   util::text_width(util::ucfirst(from->ENF().name) + "  "));
...
Map::WalkResult Map::Walk(Character *from, Direction direction, bool admin) {
  ...
  if (!admin) {
    ...
    if (this->Occupied(target_x, target_y, PlayerOnly) &&
        (from->last_walk + double(this->world->config["GhostTimer"]) >
         Timer::GetTime()))
      return WalkFail;
  }
  ...
  if (warp) {
    if (from->level >= warp.levelreq &&
        (warp.spec == Map_Warp::NoDoor || warp.open)) {
      Map *map = this->world->GetMap(warp.map);
      if (from->SourceAccess() < ADMIN_GUIDE && map->evacuate_lock &&
          map->id != from->map->id) {
  ...
  UTIL_FOREACH(this->characters, checkchar) {
    ...
    for (std::size_t i = 0; i < oldcoords.size(); ++i) {
      if (checkchar->x == oldcoords[i].first &&
          checkchar->y == oldcoords[i].second) {
        oldchars.push_back(checkchar);
      } else if (checkchar->x == newcoords[i].first &&
                 checkchar->y == newcoords[i].second) {
  ...
  UTIL_FOREACH(this->npcs, checknpc) {
    ...
    for (std::size_t i = 0; i < oldcoords.size(); ++i) {
      if (checknpc->x == oldcoords[i].first &&
          checknpc->y == oldcoords[i].second) {
        oldnpcs.push_back(checknpc);
      } else if (checknpc->x == newcoords[i].first &&
                 checknpc->y == newcoords[i].second) {
  ...
  UTIL_FOREACH(this->items, checkitem) {
    for (std::size_t i = 0; i < oldcoords.size(); ++i) {
      if (checkitem->x == newcoords[i].first &&
          checkitem->y == newcoords[i].second) {
  ...
  UTIL_FOREACH(oldnpcs, npc) { npc->RemoveFromView(from); }
  ...
  if (spike_damage > 0.0 &&
      (spec == Map_Tile::Spikes2 || spec == Map_Tile::Spikes3) &&
      !from->IsHideInvisible()) {
...
Map::WalkResult Map::Walk(NPC *from, Direction direction) {
  ...
  UTIL_FOREACH(newchars, character) { character->Send(builder); }
  ...
  UTIL_FOREACH(oldchars, character) { from->RemoveFromView(character); }

  return WalkOK;
}

void Map::Attack(Character *from, Direction direction) {
  const EIF_Data &wepdata =
      this->world->eif->Get(from->paperdoll[Character::Weapon]);
  const EIF_Data &shielddata =
      this->world->eif->Get(from->paperdoll[Character::Shield]);
  ...
  bool is_instrument =
      (wep_graphic != 0 && this->world->IsInstrument(wep_graphic));

  if (!is_instrument && (this->pk || (this->world->config["GlobalPK"] &&
                                      !this->world->PKExcept(this->id)))) {
  ...
  for (int i = 0; i < range; ++i) {
    ...
    UTIL_FOREACH(this->npcs, npc) {
      if ((npc->ENF().type == ENF::Passive ||
           npc->ENF().type == ENF::Aggressive ||
           from->SourceDutyAccess() >=
               static_cast<int>(this->world->admin_config["killnpc"])) &&
        ...
        bool critical =
            std::abs(int(npc->direction) - from->direction) != 2 ||
            rand < static_cast<double>(this->world->config["CriticalRate"]);
        ...
        {
          const ECF_Data &class_data = this->world->ecf->Get(from->clas);
          std::string class_crit_key =
              "class." + util::to_string(class_data.type) + ".crit";
          if (this->world->formulas_config.count(class_crit_key)) {
            double class_crit_mod = rpn_eval(
                rpn_parse(this->world->formulas_config[class_crit_key]),
                formula_vars);
            if (!critical && rand < static_cast<double>(
                                        this->world->config["CriticalRate"]) +
                                        class_crit_mod) {
              critical = true;
            }
          }
        ...
        amount = static_cast<int>(rpn_eval(
            rpn_parse(this->world->formulas_config["damage"]), formula_vars));
        double hit_rate = rpn_eval(
            rpn_parse(this->world->formulas_config["hit_rate"]), formula_vars);
...
bool Map::AttackPK(Character *from, Direction direction) {
  ...
  if (this->world->eif->Get(from->paperdoll[Character::Weapon]).subtype ==
      EIF::Ranged) {
    range = static_cast<int>(this->world->config["RangedDistance"]);
  }
  ...
  for (int i = 0; i < range; ++i) {
    ...
    UTIL_FOREACH(this->characters, character) {
      if (character->mapid == this->id && !character->nowhere &&
          character->x == target_x && character->y == target_y) {
        int amount = util::rand(from->mindam, from->maxdam);
        double rand = util::rand(0.0, 1.0);
        // Checks if target is facing you
        bool critical =
            std::abs(int(character->direction) - from->direction) != 2 ||
            rand < static_cast<double>(this->world->config["CriticalRate"]);
...
void Map::Sit(Character *from, SitState sit_type) {
  ...
  PacketBuilder builder((sit_type == SIT_CHAIR) ? PACKET_CHAIR : PACKET_SIT,
                        PACKET_PLAYER, 6);
  builder.AddShort(from->PlayerID());
  builder.AddChar(from->x);
  builder.AddChar(from->y);
...
bool Map::Occupied(unsigned char x, unsigned char y, Map::OccupiedTarget target,
  ...
  if (target != Map::NPCOnly) {
    UTIL_FOREACH(this->characters, character) {
      bool ghost = adminghost &&
                   (!character->CanInteractCombat() || character->IsHideNpc());
  ...
  if (Map_Warp &warp = this->GetWarp(x, y)) {
    ...
    if (from && warp.spec > Map_Warp::Door) {
      ...
      if (!from->CanInteractDoors() ||
          !from->HasItem(this->world->eif->GetKey(keynum))) {
    ...
    TimeEvent *event = new TimeEvent(map_close_door, close,
                                     this->world->config["DoorTimer"], 1);
    this->world->timer.Register(event);
...
void Map::SpellSelf(Character *from, unsigned short spell_id) {
  ...
  builder.AddChar(static_cast<unsigned char>(util::clamp<int>(
      static_cast<int>(double(from->hp) / double(from->maxhp) * 100.0), 0,
      100)));
...
void Map::SpellAttack(Character *from, NPC *npc, unsigned short spell_id) {
  ...
  if ((npc->ENF().type == ENF::Passive || npc->ENF().type == ENF::Aggressive) &&
    ...
    int amount =
        util::rand(from->mindam + spell.mindam, from->maxdam + spell.maxdam);
    double rand = util::rand(0.0, 1.0);
...
void Map::SpellAttackPK(Character *from, Character *victim,
  ...
  if (spell.type == ESF::Damage &&
      (from->map->pk ||
       (this->world->config["GlobalPK"] && !this->world->PKExcept(this->id)))) {
    if (!from->CanInteractPKCombat())
      return;
    ...
    if (!from->CanInteractCombat() && from != victim &&
        !(from->CanInteractPKCombat() &&
          (from->map->pk || (this->world->config["GlobalPK"] &&
                             !this->world->PKExcept(this->id))))) {
...
void Map::SpellGroup(Character *from, unsigned short spell_id) {
  ...
  PacketBuilder builder(PACKET_SPELL, PACKET_TARGET_GROUP,
                        8 + from->party->members.size() * 10);
  builder.AddShort(spell_id);
  builder.AddShort(from->PlayerID());
  builder.AddShort(from->tp);
  ...
  UTIL_FOREACH(from->party->members, member) {
    ...
    builder.AddChar(static_cast<unsigned char>(util::clamp<int>(
        static_cast<int>(double(member->hp) / double(member->maxhp) * 100.0), 0,
        100)));
  ...
  UTIL_FOREACH(in_range, character) { character->Send(builder); }
}

std::shared_ptr<Map_Item> Map::AddItem(short id, int amount, unsigned char x,
                                       unsigned char y, Character *from) {
  std::shared_ptr<Map_Item> newitem(
      std::make_shared<Map_Item>(0, id, amount, x, y, 0, 0));
  ...
  if (from || (from && from->SourceAccess() <= ADMIN_GM)) {
    ...
    if (ontile >= static_cast<int>(this->world->config["MaxTile"]) ||
        onmap >= static_cast<int>(this->world->config["MaxMap"])) {
      return newitem;
    }
  }
...
std::list<std::shared_ptr<Map_Item>>::iterator
Map::DelItem(std::list<std::shared_ptr<Map_Item>>::iterator it,
             Character *from) {
...
bool Map::Walkable(unsigned char x, unsigned char y, bool npc) const {
  ...
  if (this->world->config["GhostArena"] &&
      this->GetTile(x, y).tilespec == Map_Tile::Arena &&
      this->Occupied(x, y, PlayerAndNPC))
    return false;
...
Map::CharactersInRange(unsigned char x, unsigned char y, unsigned char range) {
...
std::vector<NPC *> Map::NPCsInRange(unsigned char x, unsigned char y,
                                    unsigned char range) {
  std::vector<NPC *> npcs;
...
bool Map::Evacuate() {
  if (!this->evacuate_lock) {
    ...
    evac->map = this;
    evac->step = int(evac->map->world->config["EvacuateLength"]) /
                 int(evac->map->world->config["EvacuateTick"]);

    TimeEvent *event = new TimeEvent(
        map_evacuate, evac, this->world->config["EvacuateTick"], evac->step);
    this->world->timer.Register(event);
...
bool Map::Reload() {
  ...
  UTIL_FOREACH(temp, character) {
    character->player->client->Upload(FILE_MAP, character->mapid,
                                      INIT_MAP_MUTATION);
  ...
  if (this->effect == EffectHPDrain) {
    ...
    for (Character *character : this->characters) {
      ...
      if (hpdrain_damage > 0.0) {
        ...
        PacketBuilder builder(PACKET_EFFECT, PACKET_TARGET_OTHER,
                              6 + this->characters.size() * 5);
        ...
        for (Character *other : this->characters) {
          ...
          builder.AddChar(static_cast<unsigned char>(
              util::clamp<int>(static_cast<int>(double(other->hp) /
                                                double(other->maxhp) * 100.0),
                               0, 100)));
          builder.AddShort(damage);
        }
...
void Map::TimedQuakes(bool initialize) {
  ...
  if (this->currentQuakeTick >= this->nextQuakeTick || initialize) {
    ...
    if (quakeData.size() != 4) {
      Console::Dbg("Timed quake on map %d failed - invalid parameters for %s "
                   "in config (expected 4 comma-delimited values, got: %s)",
                   this->id, configString.c_str(),
                   this->world->config[configString].GetString().c_str());
      return;
    }

    this->currentQuakeTick = 0;
    this->nextQuakeTick =
        util::rand(util::to_int(quakeData[0]), util::to_int(quakeData[1]));

    if (!initialize) {
      auto quakeStrength =
          util::rand(util::to_int(quakeData[2]), util::to_int(quakeData[3]));
