...
public:
  ...
  Timestamp(int ts) : ts(ts % max_ts) {}

  int GetHour() const { return ts / 360000; }
  int GetMinute() const { return ts / 6000 - GetHour() * 60; }
  int GetSecond() const {
    return ts / 100 - GetHour() * 3600 - GetMinute() * 60;
  }
  int GetMillisecond() const {
    return ts * 10 - GetHour() * 3600000 - GetMinute() * 60000 -
           GetSecond() * 1000;
  }
...
/**
 * Serialize a list of items in to a text format that can be restored with
 * ItemUnserialize
 */
...
/**
 * Serialize a paperdoll of 15 items in to a string that can be restored with
 * DollUnserialize
 */
...
/**
 * Serialize a list of spells in to a text format that can be restored with
 * SpellUnserialize
 */
...
public:
  ...
  NPC *pet; // NPC that is currently following the character
  ...
  bool CanInteractPKCombat() const {
    return !(nointeract & NoInteractPKCombat);
  }
  ...
  DelItem(std::list<Character_Item>::iterator, int amount);
  ...
  void Warp(short map, unsigned char x, unsigned char y,
            WarpAnimation animation = WARP_ANIMATION_NONE);
  void Refresh();
  ...
  const short &display_str, &display_intl, &display_wis, &display_agi,
      &display_con, &display_cha;
};
