...
void Item_Use(Character *character, PacketReader &reader) {
  ...
  if (character->world->pet_config.count(id)) {
    character->TogglePet(id);
    return;
  }
  ...
  case EIF::Teleport: {
    ...
    if (item.scrollmap == 0) {
      if (character->mapid == character->SpawnMap() &&
          character->x == character->SpawnX() &&
          character->y == character->SpawnY())
        break;
    } else {
      if (character->mapid == item.scrollmap && character->x == item.scrollx &&
          character->y == item.scrolly)
        break;
    }
    ...
    if (item.scrollmap == 0) {
      character->Warp(character->SpawnMap(), character->SpawnX(),
                      character->SpawnY(), WARP_ANIMATION_SCROLL);
    } else {
      character->Warp(item.scrollmap, item.scrollx, item.scrolly,
                      WARP_ANIMATION_SCROLL);
    }
  ...
  case EIF::CureCurse: {
    ...
    for (std::size_t i = 0; i < character->paperdoll.size(); ++i) {
      if (character->world->eif->Get(character->paperdoll[i]).special ==
          EIF::Cursed) {
  ...
  case EIF::EXPReward: {
    ...
    character->exp =
        std::min(character->exp,
                 static_cast<int>(character->map->world->config["MaxExp"]));

    while (character->level <
               static_cast<int>(character->map->world->config["MaxLevel"]) &&
           character->exp >=
               character->map->world->exp_table[character->level + 1]) {
      level_up = true;
      ++character->level;
      character->statpoints +=
          static_cast<int>(character->map->world->config["StatPerLevel"]);
      character->skillpoints +=
          static_cast<int>(character->map->world->config["SkillPerLevel"]);
      character->CalculateStats();
    }
...
void Item_Drop(Character *character, PacketReader &reader) {
  ...
  if (character->HasItem(id) >= amount &&
      character->mapid !=
          static_cast<int>(character->world->config["JailMap"])) {
    std::shared_ptr<Map_Item> item =
        character->map->AddItem(id, amount, x, y, character);

    if (item) {
      item->owner = character->PlayerID();
      item->unprotecttime =
          Timer::GetTime() +
          static_cast<double>(character->world->config["ProtectPlayerDrop"]);
...
void Item_Get(Character *character, PacketReader &reader) {
  ...
  if (item) {
    int distance =
        util::path_length(item->x, item->y, character->x, character->y);

    if (distance > static_cast<int>(character->world->config["DropDistance"])) {
      return;
    }

    if (item->owner != character->PlayerID() &&
        item->unprotecttime > Timer::GetTime()) {
      return;
    }
