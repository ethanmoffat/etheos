...
void world_spawn_npcs(void *world_void) {
  ...
  UTIL_FOREACH(world->maps, map) {
    UTIL_FOREACH(map->npcs, npc) {
      if ((!npc->alive &&
           npc->dead_since + (double(npc->spawn_time) * spawnrate) <
               current_time) &&
          (!npc->ENF().child || (npc->parent && npc->parent->alive &&
                                 world->config["RespawnBossChildren"]))) {
#ifdef DEBUG
...
void world_talk_npcs(void *world_void) {
  ...
  UTIL_FOREACH(world->maps, map) {
    UTIL_FOREACH(map->npcs, npc) {
      if (npc->alive &&
          npc->last_talk + npc->Data().talk_speed < current_time) {
        npc->Talk();
      }
    }
  }
  ...
  UTIL_FOREACH(world->characters, character) {
    ...
    if (character->hp != character->maxhp) {
      if (character->sitting != SIT_STAND)
        character->hp += static_cast<short>(
            character->maxhp * double(world->config["SitHPRecoverRate"]));
      else
        character->hp += static_cast<short>(
            character->maxhp * double(world->config["HPRecoverRate"]));
    ...
    if (character->tp != character->maxtp) {
      if (character->sitting != SIT_STAND)
        character->tp += static_cast<short>(
            character->maxtp * double(world->config["SitTPRecoverRate"]));
      else
        character->tp += static_cast<short>(
            character->maxtp * double(world->config["TPRecoverRate"]));
  ...
  UTIL_FOREACH(world->maps, map) {
    UTIL_FOREACH(map->npcs, npc) {
      if (npc->alive && npc->hp < npc->ENF().hp) {
        npc->hp += static_cast<int>(npc->ENF().hp *
                                    double(world->config["NPCRecoverRate"]));
...
void world_warp_suck(void *world_void) {
  struct Warp_Suck_Action {
    ...
    Warp_Suck_Action(Character *character, short map, unsigned char x,
                     unsigned char y)
  ...
  UTIL_FOREACH(world->maps, map) {
    UTIL_FOREACH(map->characters, character) {
      ...
      auto check_warp = [&](bool test, unsigned char x, unsigned char y) {
        ...
        if (!warp || warp.levelreq > character->level ||
  ...
  UTIL_FOREACH(actions, act) {
    if (act.character->SourceAccess() < ADMIN_GUIDE &&
        world->GetMap(act.map)->evacuate_lock) {
...
void world_despawn_items(void *world_void) {
  ...
  restart_loop:
    UTIL_FOREACH(map->items, item) {
      if (item->unprotecttime <
          (Timer::GetTime() -
           static_cast<double>(world->config["ItemDespawnRate"]))) {
...
void world_timed_save(void *world_void) {
  ...
  UTIL_FOREACH(world->characters, character) { character->Save(); }
...
void World::UpdateConfig() {
  ...
  std::vector<std::string> rate_list =
      util::explode(',', this->config["NPCMovementRate"]);
  ...
  std::vector<std::string> instrument_list =
      util::explode(',', this->config["InstrumentItems"]);
...
World::World(std::shared_ptr<DatabaseFactory> databaseFactory,
             const Config &eoserv_config, const Config &admin_config)
    : databaseFactory(databaseFactory), config(eoserv_config),
...
void World::Initialize() {
  ...
  this->passwordHashers[BCRYPT].reset(
      new BcryptHasher(int(this->config["BcryptWorkload"])));
  this->loginManager.reset(
      new LoginManager(databaseFactory, this->config, this->passwordHashers));
  ...
  Console::Out("%i/%i maps loaded.", loaded,
               static_cast<int>(this->maps.size()));
  ...
  Console::Out("%i/%i quests loaded.", static_cast<int>(this->quests.size()),
               max_quest);
  ...
  if (int(this->config["RecoverSpeed"]) > 0) {
    event = new TimeEvent(world_recover, this,
                          double(this->config["RecoverSpeed"]), Timer::FOREVER);
    this->timer.Register(event);
  }

  if (int(this->config["NPCRecoverSpeed"]) > 0) {
    event =
        new TimeEvent(world_npc_recover, this,
                      double(this->config["NPCRecoverSpeed"]), Timer::FOREVER);
    this->timer.Register(event);
  }
  ...
  if (this->config["ItemDespawn"]) {
    event = new TimeEvent(world_despawn_items, this,
                          static_cast<double>(this->config["ItemDespawnCheck"]),
                          Timer::FOREVER);
    this->timer.Register(event);
  }

  if (this->config["TimedSave"]) {
    event = new TimeEvent(world_timed_save, this,
                          static_cast<double>(this->config["TimedSave"]),
                          Timer::FOREVER);
    this->timer.Register(event);
  }

  if (this->config["SpikeTime"]) {
    event = new TimeEvent(world_spikes, this,
                          static_cast<double>(this->config["SpikeTime"]),
                          Timer::FOREVER);
    this->timer.Register(event);
  }

  if (this->config["DrainTime"]) {
    event = new TimeEvent(world_drains, this,
                          static_cast<double>(this->config["DrainTime"]),
                          Timer::FOREVER);
    this->timer.Register(event);
  }

  if (this->config["QuakeRate"]) {
    event = new TimeEvent(world_quakes, this,
                          static_cast<double>(this->config["QuakeRate"]),
                          Timer::FOREVER);
    this->timer.Register(event);
  }
...
void World::DumpToFile(const std::string &fileName) {
  ...
  if (existing.is_open()) {
    // right now merge means "overwrite file backup with live data if there are
    // duplicates" eventually this could become more advanced but it probably
    // isn't necessary
    existing >> dump;
    existing.close();
  }
  ...
  UTIL_FOREACH_CREF(this->characters, c) {
    auto existing =
        std::find_if(dump["characters"].begin(), dump["characters"].end(),
                     [&c](nlohmann::json check) {
                       return check.find("name") != check.end() &&
                              check["name"].get<std::string>() == c->real_name;
                     });
  ...
  // overwrite all existing map item / chest spawns. prevents possibility of
  // item dupes.
  ...
  UTIL_FOREACH_CREF(this->guildmanager->cache, guildPair) {
    ...
    if (guild) {
      auto existing =
          std::find_if(dump["guilds"].begin(), dump["guilds"].end(),
                       [&guild](nlohmann::json check) {
                         return check.find("tag") != check.end() &&
                                check["tag"].get<std::string>() == guild->tag;
                       });
      if (existing != dump["guilds"].end())
        dump["guilds"].erase(existing);

      dump["guilds"].push_back({{"tag", guild->tag},
                                {"name", guild->name},
...
void World::RestoreFromDump(const std::string &fileName) {
  ...
  try {
    ...
    for (; c_iter != dump["characters"].cend();) {
      ...
      auto exists =
          this->db->Query("SELECT `usage` FROM `characters` WHERE `name` = '$'",
                          charName.c_str());
      if (exists.Error()) {
        Console::Wrn("Error checking existence of character %s during restore. "
                     "Skipping restore.",
                     charName.c_str());
        ++c_iter;
        continue;
      ...
      if (exists.empty()) {
        dbRes = this->db->Query(
            "INSERT INTO `characters` (`name`, `title`, `account`, `home`, "
            "`fiance`, `partner`, `class`, `gender`, `race`, "
            "`hairstyle`, `haircolor`, `map`, `x`, `y`, `direction`, `level`, "
            "`admin`, `exp`, `hp`, `tp`, `str`, `int`, `wis`, `agi`, `con`, "
            "`cha`, `statpoints`, `skillpoints`, `karma`, `sitting`, `hidden`, "
            "`nointeract`, `bankmax`, `goldbank`, `usage`, `inventory`, "
            "`bank`, `paperdoll`, `spells`, `guild`, `guild_rank`, "
            "`guild_rank_string`, `quest`) "
            "VALUES ('$', '$', '$', '$', '$', '$', #, #, #, #, #, #, #, #, #, "
            "#, #, #, #, #, #, #, #, #, #, #, #, #, #, #, #, #, #, #, #, '$', "
            "'$', '$', '$', '$', #, '$', '$')",
            charName.c_str(), c["title"].get<std::string>().c_str(),
            c["account"].get<std::string>().c_str(),
            c["home"].get<std::string>().c_str(),
            c["fiance"].get<std::string>().c_str(),
            c["partner"].get<std::string>().c_str(), c["class"].get<int>(),
            c["gender"].get<int>(), c["race"].get<int>(),
            c["hairstyle"].get<int>(), c["haircolor"].get<int>(),
            c["map"].get<int>(), c["x"].get<int>(), c["y"].get<int>(),
            c["direction"].get<int>(), c["level"].get<int>(),
            c["admin"].get<int>(), c["exp"].get<int>(), c["hp"].get<int>(),
            c["tp"].get<int>(), c["str"].get<int>(), c["intl"].get<int>(),
            c["wis"].get<int>(), c["agi"].get<int>(), c["con"].get<int>(),
            c["cha"].get<int>(), c["statpoints"].get<int>(),
            c["skillpoints"].get<int>(), c["karma"].get<int>(),
            c["sitting"].get<int>(), c["hidden"].get<int>(),
            c["nointeract"].get<int>(), c["bankmax"].get<int>(),
            c["goldbank"].get<int>(), c["usage"].get<int>(),
            c["inventory"].get<std::string>().c_str(),
            c["bank"].get<std::string>().c_str(),
            c["paperdoll"].get<std::string>().c_str(),
            c["spells"].get<std::string>().c_str(),
            c["guild"].get<std::string>().c_str(), c["guildrank"].get<int>(),
            c["guildrank_str"].get<std::string>().c_str(),
            c["quest"].get<std::string>().c_str());
      }
      // if the database entry is older than the character data in the dump,
      // update the database with the dump's character data
      else if (exists.front()["usage"].GetInt() <= c["usage"].get<int>()) {
        dbRes = this->db->Query(
            "UPDATE `characters` SET `title` = '$', `home` = '$', `fiance` = "
            "'$', `partner` = '$', `class` = #, `gender` = #, `race` = #, "
            "`hairstyle` = #, `haircolor` = #, `map` = #, `x` = #, `y` = #, "
            "`direction` = #, `level` = #, `admin` = #, `exp` = #, `hp` = #, "
            "`tp` = #, "
            "`str` = #, `int` = #, `wis` = #, `agi` = #, `con` = #, `cha` = #, "
            "`statpoints` = #, `skillpoints` = #, `karma` = #, `sitting` = #, "
            "`hidden` = #, "
            "`nointeract` = #, `bankmax` = #, `goldbank` = #, `usage` = #, "
            "`inventory` = '$', `bank` = '$', `paperdoll` = '$', "
            "`spells` = '$', `guild` = '$', `guild_rank` = #, "
            "`guild_rank_string` = '$', `quest` = '$' "
            "WHERE `name` = '$'",
            c["title"].get<std::string>().c_str(),
            c["home"].get<std::string>().c_str(),
            c["fiance"].get<std::string>().c_str(),
            c["partner"].get<std::string>().c_str(), c["class"].get<int>(),
            c["gender"].get<int>(), c["race"].get<int>(),
            ...
            c["guild"].get<std::string>().c_str(), c["guildrank"].get<int>(),
            c["guildrank_str"].get<std::string>().c_str(),
            c["quest"].get<std::string>().c_str(), charName.c_str());
      }
    ...
    for (; g_iter != dump["guilds"].cend();) {
      ...
      auto exists = this->db->Query(
          "SELECT COUNT(1) AS `count` FROM `guilds` WHERE `tag` = '$'",
          guildTag.c_str());
      if (exists.Error()) {
        Console::Wrn("Error checking existence of guild %s during restore. "
                     "Skipping restore.",
                     guildTag.c_str());
        ++g_iter;
        continue;
      ...
      if (exists.empty() || exists.front()["count"].GetInt() != 1) {
        dbRes = this->db->Query(
            "INSERT INTO `guilds` (`tag`, `name`, `description`, `ranks`, "
            "`bank`) VALUES ('$', '$', '$', '$', #)",
            guildTag.c_str(), guildName.c_str(),
            g["description"].get<std::string>().c_str(),
            g["ranks"].get<std::string>().c_str(), g["bank"].get<int>());
      } else {
        dbRes = this->db->Query("UPDATE `guilds` SET `description` = '$', "
                                "`ranks` = '$', `bank` = # WHERE tag = '$'",
                                g["description"].get<std::string>().c_str(),
                                g["ranks"].get<std::string>().c_str(),
                                g["bank"].get<int>(), guildTag.c_str());
      }

      if (!dbRes.Error()) {
#ifdef DEBUG
        Console::Dbg("Restored guild:     %s (%s)", guildName.c_str(),
                     guildTag.c_str());
#endif
    ...
    Console::Err("Database operation failed during restore. %s: %s", dbe.what(),
                 dbe.error());
    this->db->Rollback();
  }

  UTIL_FOREACH_CREF(dump["mapState"]["items"], i) {
    auto map = std::find_if(this->maps.begin(), this->maps.end(), [&i](Map *m) {
      return m->id == i["mapId"].get<int>();
    });
    if (map == this->maps.end())
      continue;

    (*map)->items.push_back(std::make_shared<Map_Item>(
        i["uid"].get<short>(), i["itemId"].get<short>(), i["amount"].get<int>(),
        i["x"].get<unsigned char>(), i["y"].get<unsigned char>(), 0, 0));

#ifdef DEBUG
    Console::Dbg("Restored item:     %dx%d", i["itemId"].get<int>(),
                 i["amount"].get<int>());
#endif
  }
  ...
  UTIL_FOREACH_CREF(dump["mapState"]["chests"], chest) {
    auto map =
        std::find_if(this->maps.begin(), this->maps.end(), [&chest](Map *m) {
          return m->id == chest["mapId"].get<int>();
        });
    if (map == this->maps.end())
      continue;
    ...
    auto mapChest = std::find_if((*map)->chests.begin(), (*map)->chests.end(),
                                 [&chest](std::shared_ptr<Map_Chest> check) {
                                   return check->x == chest["x"].get<int>() &&
                                          check->y == chest["y"].get<int>();
                                 });
    if (mapChest == (*map)->chests.end())
      continue;

    (*mapChest)->AddItem(chest["itemId"].get<int>(), chest["amount"].get<int>(),
                         chest["slot"].get<int>());

#ifdef DEBUG
    Console::Dbg("Restored chest:    %dx%d", chest["itemId"].get<int>(),
                 chest["amount"].get<int>());
#endif
  }
  ...
  if (dump["characters"].empty() && dump["guilds"].empty()) {
    ...
    if (!rewrite.is_open()) {
      throw std::runtime_error("Unable to update the world dump file after "
                               "partially restoring world state");
    }
...
void World::LoadHome() {
  ...
  UTIL_FOREACH(temp_homes, home) { this->homes.push_back(home.second); }
}

void World::LoadPets() {
  this->pet_config.clear();

  Config pets_config;

  try {
    pets_config.Read("./data/pets.ini");
  } catch (std::runtime_error &e) {
    Console::Wrn("Could not load pets.ini: %s", e.what());
    return;
  }

  UTIL_FOREACH(pets_config, pc) {
    int item_id = util::to_int(pc.first);
    int npc_id = util::to_int(pc.second);
    this->pet_config[item_id] = npc_id;
  }

  Console::Out("%i pets loaded.", this->pet_config.size());
}

int World::GenerateCharacterID() { return ++this->last_character_id; }

unsigned short World::GenerateOperationID(
    std::function<unsigned short(const EOClient *)> get_id) const {
  unsigned short candidate_id =
      static_cast<unsigned int>(util::rand(20000, 60000)) - 1;
  std::list<Client *>::const_iterator matching_client;

  do {
    candidate_id++;
    matching_client = std::find_if(
        this->server->clients.cbegin(), this->server->clients.cend(),
...
void World::Login(Character *character) {
  ...
  if (this->GetMap(character->mapid)->relog_x ||
      this->GetMap(character->mapid)->relog_y) {
  ...
  if (character->sitting == SIT_CHAIR) {
    ...
    if (spec == Map_Tile::ChairDown)
      ...
      character->direction = character->direction == DIRECTION_RIGHT
                                 ? DIRECTION_RIGHT
                                 : DIRECTION_DOWN;
    else if (spec == Map_Tile::ChairUpLeft)
      character->direction = character->direction == DIRECTION_LEFT
                                 ? DIRECTION_LEFT
                                 : DIRECTION_UP;
...
void World::Logout(Character *character) {
  ...
  this->characters.erase(std::remove(UTIL_RANGE(this->characters), character),
                         this->characters.end());
}
...
void World::Msg(Command_Source *from, std::string message, bool echo) {
  ...
  message = util::text_cap(
      message, static_cast<int>(this->config["ChatMaxWidth"]) -
                   util::text_width(util::ucfirst(from_str) + "  "));

  PacketBuilder builder(PACKET_TALK, PACKET_MSG,
                        2 + from_str.length() + message.length());
...
void World::AdminMsg(Command_Source *from, std::string message, int minlevel,
                     bool echo) {
  std::string from_str = from ? from->SourceName() : "server";
...
void World::ServerMsg(std::string message) {
  message =
      util::text_cap(message, static_cast<int>(this->config["ChatMaxWidth"]) -
                                  util::text_width("Server  "));

  PacketBuilder builder(PACKET_TALK, PACKET_SERVER, message.length());
  builder.AddString(message);

  UTIL_FOREACH(this->characters, character) { character->Send(builder); }
}

void World::AdminReport(Character *from, std::string reportee,
                        std::string message) {
  message = util::text_cap(
      message, static_cast<int>(this->config["ChatMaxWidth"]) -
                   util::text_width(util::ucfirst(from->SourceName()) +
                                    "  reports: " + reportee + ", "));

  PacketBuilder builder(PACKET_ADMININTERACT, PACKET_REPLY,
                        5 + from->SourceName().length() + message.length() +
                            reportee.length());
  ...
  UTIL_FOREACH(this->characters, character) {
    if (character->SourceAccess() >=
        static_cast<int>(this->admin_config["reports"])) {
      character->Send(builder);
    }
  }
  ...
  if (static_cast<std::size_t>(boardid) < this->boards.size()) {
    ...
    newpost->subject = std::string(" [Report] ") +
                       util::ucfirst(from->SourceName()) +
                       " reports: " + reportee;
    ...
    if (this->config["LogReports"]) {
      try {
        this->db->Query(
            ...
            "`chat_log`) VALUES ('$', '$', '$', #, '$')",
            from->SourceName().c_str(), reportee.c_str(), message.c_str(),
            int(std::time(0)), chat_log_dump.c_str());
      } catch (Database_Exception &e) {
    ...
    if (admin_board->posts.size() > static_cast<std::size_t>(static_cast<int>(
                                        this->config["AdminBoardLimit"]))) {
...
void World::AdminRequest(Character *from, std::string message) {
  ...
  PacketBuilder builder(PACKET_ADMININTERACT, PACKET_REPLY,
                        4 + from->SourceName().length() + message.length());
  ...
  short boardid =
      static_cast<int>(this->server->world->config["AdminBoard"]) - 1;
  ...
  if (static_cast<std::size_t>(boardid) < this->server->world->boards.size()) {
    ...
    newpost->author = from->SourceName();
    newpost->author_admin = from->admin;
    newpost->subject = std::string(" [Request] ") +
                       util::ucfirst(from->SourceName()) + " needs help";
    newpost->body = message;
    newpost->time = Timer::GetTime();

    admin_board->posts.push_front(newpost);

    if (admin_board->posts.size() >
        static_cast<std::size_t>(
            static_cast<int>(this->server->world->config["AdminBoardLimit"]))) {
      admin_board->posts.pop_back();
    }
  }
}
...
void World::Rehash() {
  ...
  UTIL_FOREACH(this->maps, map) { map->LoadArena(); }
...
void World::ReloadPub(bool quiet) {
  ...
  if (eif_id != this->eif->rid || enf_id != this->enf->rid ||
    ...
    if (!quiet) {
      UTIL_FOREACH(this->characters, character) {
        character->ServerMsg(
            "The server has been reloaded, please log out and in again.");
      }
    }
  }
...
void World::ReloadQuests() {
  ...
  UTIL_FOREACH(this->characters, c) { c->quests.clear(); }
  ...
  UTIL_FOREACH(this->characters, c) {
    ...
    UTIL_FOREACH(c->quests_inactive, state) {
      ...
      try {
        ...
        Console::Wrn(ex.what());
        Console::Wrn("Could not resume quest: %i. Marking as inactive.",
                     state.quest_id);
      ...
      auto result = c->quests.insert(
          std::make_pair(state.quest_id, std::move(quest_context)));
...
Map *World::GetMap(short id) {
  try {
    ...
    try {
      ...
      throw std::runtime_error("Map #" + util::to_string(id) +
                               " and fallback map #1 are unavailable");
    }
  }
}
...
bool World::CharacterExists(std::string name) {
  Database_Result res = this->db->Query(
      "SELECT 1 FROM `characters` WHERE `name` = '$'", name.c_str());
  return !res.empty();
}

Character *World::CreateCharacter(Player *player, std::string name,
  ...
  if (static_cast<int>(this->config["StartMap"])) {
    ...
    std::snprintf(buffer, 1024, ",%i,%i,%i",
                  static_cast<int>(this->config["StartMap"]),
                  static_cast<int>(this->config["StartX"]),
                  static_cast<int>(this->config["StartY"]));
    startmapval = buffer;
  }
  ...
  this->db->Query(
      "INSERT INTO `characters` (`name`, `account`, `gender`, `hairstyle`, "
      "`haircolor`, `race`, `inventory`, `bank`, `paperdoll`, `spells`, "
      "`quest`, `vars`@) VALUES ('$','$',#,#,#,#,'$','','$','$','',''@)",
      startmapinfo.c_str(), name.c_str(), player->username.c_str(), gender,
      hairstyle, haircolor, race,
      static_cast<std::string>(this->config["StartItems"]).c_str(),
      static_cast<std::string>(gender ? this->config["StartEquipMale"]
                                      : this->config["StartEquipFemale"])
          .c_str(),
      static_cast<std::string>(this->config["StartSpells"]).c_str(),
      startmapval.c_str());
...
World::CheckCredential(EOClient *client) {
  if (this->loginManager->LoginBusy()) {
    return AsyncOperation<AccountCredentials, LoginReply>::FromResult(
        LOGIN_BUSY, client, LOGIN_OK);
  }

  return this->loginManager->CheckLoginAsync(client);
}

AsyncOperation<PasswordChangeInfo, bool> *
World::ChangePassword(EOClient *client) {
  return this->loginManager->SetPasswordAsync(client);
}

AsyncOperation<AccountCreateInfo, bool> *
World::CreateAccount(EOClient *client) {
  return this->loginManager->CreateAccountAsync(client);
}

bool World::PlayerExists(std::string username) {
  Database_Result res = this->db->Query(
      "SELECT 1 FROM `accounts` WHERE `username` = '$'", username.c_str());
  return !res.empty();
}
...
bool World::GetPendingLogin(const std::string &username) {
  ...
  try {
    result =
        this->pending_logins.find(username) != this->pending_logins.end() &&
...
void World::Kick(Command_Source *from, Character *victim, bool announce) {
  if (announce)
    this->ServerMsg(i18n.Format("announce_removed", victim->SourceName(),
                                from ? from->SourceName() : "server",
                                i18n.Format("kicked")));
...
void World::Jail(Command_Source *from, Character *victim, bool announce) {
  ...
  victim->Warp(static_cast<int>(this->config["JailMap"]),
               static_cast<int>(this->config["JailX"]),
               static_cast<int>(this->config["JailY"]),
               bubbles ? WARP_ANIMATION_ADMIN : WARP_ANIMATION_NONE);
}
...
void World::Unjail(Command_Source *from, Character *victim) {
  ...
  victim->Warp(static_cast<int>(this->config["JailMap"]),
               static_cast<int>(this->config["UnJailX"]),
               static_cast<int>(this->config["UnJailY"]),
               bubbles ? WARP_ANIMATION_ADMIN : WARP_ANIMATION_NONE);
}

void World::Ban(Command_Source *from, Character *victim, int duration,
                bool announce) {
  std::string from_str = from ? from->SourceName() : "server";

  if (announce)
    this->ServerMsg(i18n.Format("announce_removed", victim->SourceName(),
                                from_str, i18n.Format("banned")));

  std::string query(
      "INSERT INTO bans (username, ip, hdid, expires, setter) VALUES ");

  query += "('" + db->Escape(victim->player->username) + "', ";
  query += util::to_string(
               static_cast<int>(victim->player->client->GetRemoteAddr())) +
           ", ";
...
void World::Mute(Command_Source *from, Character *victim, bool announce) {
  if (announce && !this->config["SilentMute"])
    this->ServerMsg(i18n.Format("announce_muted", victim->SourceName(),
                                from ? from->SourceName() : "server",
                                i18n.Format("banned")));
...
int World::CheckBan(const std::string *username, const IPAddress *address,
                    const int *hdid) {
  std::string query(
      "SELECT COALESCE(MAX(expires),-1) AS expires FROM bans WHERE (");
  ...
  if (address) {
    query += "ip = ";
    query +=
        util::to_string(static_cast<int>(*const_cast<IPAddress *>(address)));
    query += " OR ";
  }
  ...
  Database_Result res = db->Query((query.substr(0, query.length() - 4) +
                                   ") AND (expires > # OR expires = 0)")
                                      .c_str(),
                                  int(std::time(0)));
...
bool World::PKExcept(const Map *map) { return this->PKExcept(map->id); }
...
bool World::PKExcept(int mapid) {
  ...
  return std::find(except_list.begin(), except_list.end(), mapid) !=
         except_list.end();
}

bool World::IsInstrument(int graphic_id) {
  return std::find(UTIL_RANGE(this->instrument_ids), graphic_id) !=
         this->instrument_ids.end();
}

World::~World() {
  UTIL_FOREACH(this->maps, map) { delete map; }

  UTIL_FOREACH(this->boards, board) { delete board; }
