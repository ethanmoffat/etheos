...
std::list<Character_Item> ItemUnserialize(const std::string &serialized) {
  ...
  UTIL_FOREACH(parts, part) {
    ...
    if (id < 1 || id > 65535 || amount < 1) {
      Console::Wrn("Discarding invalid inventory data: id: %d, amount: %d", id,
                   amount);
      continue;
    }
...
std::list<Character_Spell> SpellUnserialize(const std::string &serialized) {
  ...
  UTIL_FOREACH(parts, part) {
    ...
    if (id < 1 || id > 65535 || level < 0) {
      Console::Wrn("Discarding invalid spell data: id: %d, level: %d", id,
                   level);
      continue;
    }
...
std::string
QuestSerialize(const std::map<short, std::shared_ptr<Quest_Context>> &list,
               const std::set<Character_QuestState> &list_inactive) {
  std::string serialized;
...
#ifdef DEBUG
      Console::Dbg(
          "Discarding inactive quest save as the quest was restarted: %i",
          state.quest_id);
#endif // DEBUG
      continue;
    }
...
void QuestUnserialize(std::string serialized, Character *character) {
  ...
  UTIL_FOREACH(parts, part) {
    ...
    auto result = character->quests.insert(
...
std::vector<std::string> BotListUnserialize(std::string serialized) {
  ...
  std::transform(UTIL_CRANGE(bots), bots.begin(),
                 [](std::string s) { return util::lowercase(util::trim(s)); });
  return bots;
}

template <typename T>
static T GetRow(std::unordered_map<std::string, util::variant> &row,
                const char *col) {
  return row[col];
}

Character::Character(World *world)
    : online(false), world(world),
...
Character::Character(std::string name, World *world)
    : muted_until(0), bot(false), cosmetic_paperdoll{{}}, world(world),
      display_str(this->world->config["UseAdjustedStats"] ? adj_str : str),
      display_intl(this->world->config["UseAdjustedStats"] ? adj_intl : intl),
      display_wis(this->world->config["UseAdjustedStats"] ? adj_wis : wis),
      display_agi(this->world->config["UseAdjustedStats"] ? adj_agi : agi),
      ...
      pet(nullptr) {
  {
    std::vector<std::string> bot_characters =
        BotListUnserialize(this->world->config["BotCharacters"]);
    auto bot_it = std::find(UTIL_CRANGE(bot_characters), util::lowercase(name));
    this->bot = bot_it != bot_characters.end();
  }

  Database_Result res = this->world->db->Query(
      "SELECT `name`, `title`, `home`, `fiance`, `partner`, `admin`, `class`, "
      "`gender`, `race`, `hairstyle`, `haircolor`,"
      "`map`, `x`, `y`, `direction`, `level`, `exp`, `hp`, `tp`, `str`, `int`, "
      "`wis`, `agi`, `con`, `cha`, `statpoints`, `skillpoints`, "
      "`karma`, `sitting`, `hidden`, `bankmax`, `goldbank`, `usage`, "
      "`inventory`, `bank`, `paperdoll`, `spells`, `guild`, `guild_rank`, "
      "`guild_rank_string`, `quest`, `vars`, "
  ...
  if (this->admin >=
          static_cast<int>(world->config["NoInteractDefaultAdmin"]) &&
      !(this->nointeract & NoInteractCustom)) {
    this->nointeract = static_cast<int>(world->config["NoInteractDefault"]);
  }
}

unsigned int Character::PlayerID() const { return this->player->id; }
...
void Character::Msg(Character *from, std::string message) {
  message = util::text_cap(
      message, static_cast<int>(this->world->config["ChatMaxWidth"]) -
  ...
  PacketBuilder builder(PACKET_TALK, PACKET_TELL,
                        2 + from->SourceName().length() + message.length());
...
void Character::Sit(SitState sit_type) { this->map->Sit(this, sit_type); }

void Character::Stand() { this->map->Stand(this); }
...
void Character::PlayBard(unsigned char instrument, unsigned char note,
                         bool echo) {
...
short Character::SpellLevel(short spell) {
  auto it = std::find_if(UTIL_RANGE(this->spells),
                         [&](Character_Spell cs) { return cs.id == spell; });
...
bool Character::AddItem(short item, int amount) {
  ...
  if (item <= 0 ||
      static_cast<std::size_t>(item) >= this->world->eif->data.size()) {
    return false;
  }
...
std::list<Character_Item>::iterator
Character::DelItem(std::list<Character_Item>::iterator it, int amount) {
  if (amount <= 0) {
    return ++it;
  }
...
int Character::CanHoldItem(short itemid, int max_amount) {
  ...
  if (int(this->world->config["EnforceWeight"]) >= 2 &&
      SourceDutyAccess() <
          static_cast<int>(world->admin_config["unlimitedweight"])) {
    ...
    if (this->weight > this->maxweight)
      ...
      amount =
          std::min((this->maxweight - this->weight) / item.weight, max_amount);
  }
...
bool Character::AddTradeItem(short item, int amount) {
  ...
  if (trade_add_quantity) {
    ...
    if (tradeitem + amount < 0 ||
        tradeitem + amount > int(this->world->config["MaxTrade"])) {
      return false;
    }
  }
...
bool Character::DelTradeItem(short item) {
  for (std::list<Character_Item>::iterator it = this->trade_inventory.begin();
       it != this->trade_inventory.end(); ++it) {
    if (it->id == item) {
...
bool Character::DelSpell(short spell) {
  auto remove_it =
      std::remove_if(UTIL_RANGE(this->spells),
                     [&](Character_Spell cs) { return cs.id == spell; });
...
void Character::SpellAct() {
  ...
  case TargetNPC:
    ...
    npc_victim =
        this->map->GetNPCIndex(static_cast<unsigned char>(spell_target_id));
...
double Character::SpellCooldownTime() const {
  ...
  double cooldown =
      (this->spell_fired_time + cooldown_period) - Timer::GetTime();
...
bool Character::Unequip(short item, unsigned char subloc) {
  ...
  for (std::size_t i = 0; i < this->paperdoll.size(); ++i) {
    if (this->paperdoll[i] == item) {
      if (((i == Character::Ring2 || i == Character::Armlet2 ||
            i == Character::Bracer2)
               ? 1
               : 0) == subloc) {
        this->paperdoll[i] = 0;
...
static bool character_equip_oneslot(Character *character, short item,
                                    unsigned char subloc,
                                    Character::EquipLocation slot) {
  (void)subloc;
...
static bool character_equip_twoslot(Character *character, short item,
                                    unsigned char subloc,
                                    Character::EquipLocation slot1,
                                    Character::EquipLocation slot2) {
  if (subloc == 0) {
...
bool Character::Equip(short item, unsigned char subloc) {
  ...
  if (eif.type == EIF::Weapon && eif.subtype == EIF::TwoHanded) {
    if (this->paperdoll[Shield]) {
      const EIF_Data &shield_eif =
          this->world->eif->Get(this->paperdoll[Shield]);

      if (eif.dual_wield_dollgraphic || (shield_eif.subtype != EIF::Arrows &&
                                         shield_eif.subtype != EIF::Wings)) {
  ...
  if (eif.type == EIF::Shield) {
    if (this->paperdoll[Weapon]) {
      const EIF_Data &weapon_eif =
          this->world->eif->Get(this->paperdoll[Weapon]);

      if (weapon_eif.subtype == EIF::TwoHanded &&
          (weapon_eif.dual_wield_dollgraphic ||
           (eif.subtype != EIF::Arrows && eif.subtype != EIF::Wings))) {
  ...
  if (this->level < eif.levelreq ||
      (this->clas != eif.classreq && ecf.base != eif.classreq) ||
...
bool Character::InRange(unsigned char x, unsigned char y) const {
  return util::path_length(this->x, this->y, x, y) <=
         static_cast<int>(this->world->config.at("SeeDistance"));
}
...
void Character::Warp(short map, unsigned char x, unsigned char y,
                     WarpAnimation animation) {
  if (map <= 0 || map > int(this->world->maps.size()) ||
      !this->world->GetMap(map)->exists) {
    return;
  }
  ...
  if (pet_npc_id != 0) {
    NPC *new_pet = new NPC(this->map, pet_npc_id, this->x, this->y, 0, 0, 0);
    new_pet->owner = this;
    new_pet->ghost = true;
    new_pet->spawn_type = 0;
    new_pet->act_speed = double(this->world->config["PacketRateWalk"]);

    this->pet = new_pet;
    this->map->AddNPC(new_pet);
    new_pet->Spawn();
  }
...
void Character::Refresh() {
  ...
  PacketBuilder builder(PACKET_REFRESH, PACKET_REPLY,
                        3 + updatecharacters.size() * 60 +
                            updatenpcs.size() * 6 + updateitems.size() * 9);
  builder.AddChar(
      static_cast<char>(updatecharacters.size())); // Number of players
  builder.AddByte(255);
...
void Character::ShowBoard(Board *board) {
  ...
  PacketBuilder builder(
      PACKET_BOARD, PACKET_OPEN,
      2 + board->posts.size() *
              (17 + int(this->world->config["BoardMaxSubjectLength"]) +
               date_res));
  ...
  UTIL_FOREACH(board->posts, post) {
    if (post->author == this->player->character->SourceName()) {
      ...
      if (post->time +
              static_cast<int>(this->world->config["BoardRecentPostTime"]) >
          Timer::GetTime()) {
        ++recent_post_count;
      }
    }
  ...
  int posts_remaining = std::min(
      static_cast<int>(this->world->config["BoardMaxUserPosts"]) - post_count,
      static_cast<int>(this->world->config["BoardMaxUserRecentPosts"]) -
          recent_post_count);
...
std::string Character::GuildRankString() {
  ...
  if (this->world->config["GuildCustomRanks"] &&
      !this->guild_rank_string.empty()) {
...
short Character::SpawnMap() { return this->world->GetHome(this)->map; }

unsigned char Character::SpawnX() { return this->world->GetHome(this)->x; }

unsigned char Character::SpawnY() { return this->world->GetHome(this)->y; }
...
void Character::CalculateStats(bool trigger_quests) {
  ...
  this->maxhp += static_cast<short>(
      rpn_eval(rpn_parse(this->world->formulas_config["hp"]), formula_vars));
  this->maxtp += static_cast<short>(
      rpn_eval(rpn_parse(this->world->formulas_config["tp"]), formula_vars));
  this->maxsp += static_cast<short>(
      rpn_eval(rpn_parse(this->world->formulas_config["sp"]), formula_vars));
  this->maxweight = static_cast<short>(rpn_eval(
      rpn_parse(this->world->formulas_config["weight"]), formula_vars));
  ...
  if (this->world->config["UseClassFormulas"]) {
    auto dam = static_cast<short>(rpn_eval(
        rpn_parse(
            this->world->formulas_config["class." + util::to_string(ecf.type) +
                                         ".damage"]),
        formula_vars));

    this->mindam += dam;
    this->maxdam += dam;
    this->armor += static_cast<short>(rpn_eval(
        rpn_parse(
            this->world->formulas_config["class." + util::to_string(ecf.type) +
                                         ".defence"]),
        formula_vars));
    this->accuracy += static_cast<short>(rpn_eval(
        rpn_parse(
            this->world->formulas_config["class." + util::to_string(ecf.type) +
                                         ".accuracy"]),
        formula_vars));
    this->evade += static_cast<short>(rpn_eval(
        rpn_parse(
            this->world->formulas_config["class." + util::to_string(ecf.type) +
                                         ".evade"]),
        formula_vars));
...
void Character::DropAll(Character *killer) {
  ...
  while (it != this->inventory.end()) {
    ...
    std::shared_ptr<Map_Item> map_item = this->player->character->map->AddItem(
        it->id, it->amount, this->x, this->y, 0);

    if (map_item) {
      if (killer) {
        map_item->owner = killer->PlayerID();
        map_item->unprotecttime =
            Timer::GetTime() +
            static_cast<double>(this->world->config["ProtectPKDrop"]);
      } else {
  ...
  UTIL_FOREACH(this->paperdoll, id) {
    if (id == 0 || this->world->eif->Get(id).special == EIF::Lore ||
        this->world->eif->Get(id).special == EIF::Cursed) {
      ++i;
      continue;
    }

    std::shared_ptr<Map_Item> map_item =
        this->player->character->map->AddItem(id, 1, this->x, this->y, 0);
...
void Character::Hide(int setflags) {
  ...
  if (!washidden && (setflags & HideInvisible)) {
    ...
    UTIL_FOREACH(this->map->characters, character) { character->Send(builder); }
  }
}
...
void Character::ResetQuest(short id) { this->quests[id].reset(); }
...
void Character::SpikeDamage(int amount) {
  ...
  builder3.AddChar(static_cast<unsigned char>(util::clamp<int>(
      static_cast<int>(double(this->hp) / double(this->maxhp) * 100.0), 0,
      100)));
  ...
  for (Character *watcher : this->map->CharactersInRange(
           this->x, this->y,
           static_cast<int>(this->world->config["SeeDistance"]))) {
    if (watcher == this)
      continue;
...
void Character::DeathRespawn() {
  this->hp =
      int(this->maxhp *
          static_cast<double>(this->world->config["DeathRecover"]) / 100.0);
  ...
  this->player->client->queue.AddAction(
      PacketReader(std::array<char, 2>{
...
void Character::Mute(const Command_Source *by) {
  this->muted_until =
      static_cast<int>(time(0)) + int(this->world->config["MuteLength"]);
...
void Character::FormulaVars(std::unordered_map<std::string, double> &vars,
                            std::string prefix) {
  v(level) vv(exp, "experience") v(hp) v(maxhp) v(tp) v(maxtp) v(maxsp)
      v(weight) v(maxweight) v(karma) v(mindam) v(maxdam) vv(adj_str, "str")
          vv(adj_intl, "int") vv(adj_wis, "wis") vv(adj_agi, "agi")
              vv(adj_con, "con") vv(adj_cha, "cha") vv(str, "base_str")
                  vv(intl, "base_int") vv(wis, "base_wis") vv(agi, "base_agi")
                      vv(con, "base_con") vv(cha, "base_cha") v(display_str)
                          vv(display_intl, "display_int") v(display_wis)
                              v(display_agi) v(display_con) v(display_cha)
                                  v(accuracy) v(evade) v(armor) v(admin) v(bot)
                                      v(usage) vv(clas, "class") v(gender)
                                          v(race) v(hairstyle) v(haircolor)
                                              v(mapid) v(x) v(y) v(direction)
                                                  v(sitting) v(hidden)
                                                      v(whispers) v(goldbank)
                                                          v(statpoints)
                                                              v(skillpoints)
}
...
void Character::AddPaperdollData(PacketBuilder &builder, const char *format) {
  const EIF_Data &wep =
      this->world->eif->Get(this->paperdoll[Character::Weapon]);

  unsigned short boots =
      this->world->eif->Get(this->paperdoll[Character::Boots]).dollgraphic;
  unsigned short armor =
      this->world->eif->Get(this->paperdoll[Character::Armor]).dollgraphic;
  unsigned short hat =
      this->world->eif->Get(this->paperdoll[Character::Hat]).dollgraphic;
  unsigned short weapon = wep.dollgraphic;
  unsigned short shield =
      this->world->eif->Get(this->paperdoll[Character::Shield]).dollgraphic;
...
void Character::AddChatLog(std::string marker, std::string name,
                           std::string msg) {
...
#ifdef DEBUG
  Console::Dbg("Saving character '%s' (session lasted %i minutes)",
               this->real_name.c_str(),
               int(std::time(0) - this->login_time) / 60);
#endif // DEBUG
  this->world->db->Query(
      "UPDATE `characters` SET `title` = '$', `home` = '$', `fiance` = '$', "
      "`partner` = '$', `admin` = #, `class` = #, `gender` = #, `race` = #, "
      "`hairstyle` = #, `haircolor` = #, `map` = #, `x` = #, `y` = #, "
      "`direction` = #, `level` = #, `exp` = #, `hp` = #, `tp` = #, "
      "`str` = #, `int` = #, `wis` = #, `agi` = #, `con` = #, `cha` = #, "
      "`statpoints` = #, `skillpoints` = #, `karma` = #, `sitting` = #, "
      "`hidden` = #, "
      "`nointeract` = #, `bankmax` = #, `goldbank` = #, `usage` = #, "
      "`inventory` = '$', `bank` = '$', `paperdoll` = '$', "
      "`spells` = '$', `guild` = '$', `guild_rank` = #, `guild_rank_string` = "
      "'$', `quest` = '$', `vars` = '$' WHERE `name` = '$'",
      this->title.c_str(), this->home.c_str(), this->fiance.c_str(),
      this->partner.c_str(), int(this->admin), this->clas, int(this->gender),
      int(this->race), this->hairstyle, this->haircolor, this->mapid, this->x,
      this->y, int(this->direction), this->level, this->exp, this->hp, this->tp,
      this->str, this->intl, this->wis, this->agi, this->con, this->cha,
      this->statpoints, this->skillpoints, this->karma, int(this->sitting),
      int(this->hidden), nointeract, this->bankmax, this->goldbank,
      this->Usage(), ItemSerialize(this->inventory).c_str(),
      ItemSerialize(this->bank).c_str(), DollSerialize(this->paperdoll).c_str(),
      SpellSerialize(this->spells).c_str(),
      (this->guild ? this->guild->tag.c_str() : ""), this->guild_rank,
      this->guild_rank_string.c_str(), quest_data.c_str(), "",
      this->real_name.c_str());
}
...
AdminLevel Character::SourceDutyAccess() const { return admin; }

std::string Character::SourceName() const {
  return faux_name.empty() ? real_name : faux_name;
}

Character *Character::SourceCharacter() { return this; }

World *Character::SourceWorld() { return this->world; }

Character::~Character() { this->Logout(); }

void Character::TogglePet(short item_id) {
  auto it = this->world->pet_config.find(item_id);
  if (it == this->world->pet_config.end()) {
    return;
  }

  short npc_id = it->second;
  bool toggle_off = false;

  if (this->pet) {
    if (this->pet->id == npc_id) {
      toggle_off = true;
    }
  ...
  NPC *new_pet = new NPC(this->map, npc_id, this->x, this->y, 0, 0, 0);
  new_pet->owner = this;
  new_pet->ghost = true;
  new_pet->spawn_type = 0;
  new_pet->act_speed = double(this->world->config["PacketRateWalk"]);
