...
NPC::NPC(Map *map, short id, unsigned char x, unsigned char y,
         unsigned char spawn_type, short spawn_time, unsigned char index,
         bool temporary) {
  this->id = id;
  this->map = map;
...
const ENF_Data &NPC::ENF() const { return this->Data().ENF(); }
  ...
  if (this->spawn_type < 7) {
    ...
    for (int i = 0; i < 200; ++i) {
      ...
      if (this->map->Walkable(this->x, this->y, true) &&
          (i > 100 || !this->map->Occupied(this->x, this->y, Map::NPCOnly))) {
    ...
    if (!found) {
      Console::Wrn("An NPC on map %i at %i,%i is being placed by linear scan "
                   "of spawn area (%s)",
                   this->map->id, this->spawn_x, this->spawn_y,
                   this->map->world->enf->Get(this->id).name.c_str());
      for (this->x = this->spawn_x - 2; this->x <= spawn_x + 2; ++this->x) {
        for (this->y = this->spawn_y - 2; this->y <= this->spawn_y + 2;
             ++this->y) {
          if (this->map->Walkable(this->x, this->y, true)) {
...
void NPC::Act() {
  ...
  this->last_act += double(util::rand(int(this->act_speed * 750.0),
                                      int(this->act_speed * 1250.0))) /
                    1000.0;
  ...
  if (this->owner) {
    ...
    if (!this->owner->online) {
      this->alive = false;
      this->owner->pet = nullptr; // Ensure character knows pet is gone?
      // Better handled in Character::Logout or destructor
      return;
    }

    // Calculate target position (behind owner)
    int target_x = this->owner->x;
    int target_y = this->owner->y;
    ...
    if (!this->map->Walkable(target_x, target_y)) {
      target_x = this->owner->x;
      target_y = this->owner->y;
    }

    int distance = util::path_length(target_x, target_y, this->x, this->y);

    if (distance > 0) {
      int xdiff = this->x - target_x;
      int ydiff = this->y - target_y;
      ...
      // Already there, face same direction as owner? Or just stay put.
      // Requirement said "followed the player", implies looking at them or just
      // being there. Let's face the owner if we are behind them? "Pets would
      // follow the character around (would be at the back of the character)" No
      // strict facing requirement.
    }
  ...
  unsigned char attacker_distance =
      static_cast<int>(this->map->world->config["NPCChaseDistance"]);
  ...
  if (this->ENF().type == ENF::Passive || this->ENF().type == ENF::Aggressive) {
    UTIL_FOREACH_CREF(this->damagelist, opponent) {
      if (opponent->attacker->map != this->map || opponent->attacker->nowhere ||
          opponent->last_hit <
              Timer::GetTime() -
                  static_cast<double>(
                      this->map->world->config["NPCBoredTimer"])) {
        continue;
      }

      int distance = util::path_length(opponent->attacker->x,
                                       opponent->attacker->y, this->x, this->y);

      if (distance == 0)
        distance = 1;

      if ((distance < attacker_distance) ||
          (distance == attacker_distance &&
           opponent->damage > attacker_damage)) {
    ...
    if (this->parent) {
      UTIL_FOREACH_CREF(this->parent->damagelist, opponent) {
        if (opponent->attacker->map != this->map ||
            opponent->attacker->nowhere ||
            opponent->last_hit <
                Timer::GetTime() -
                    static_cast<double>(
                        this->map->world->config["NPCBoredTimer"])) {
        ...
        int distance = util::path_length(
            opponent->attacker->x, opponent->attacker->y, this->x, this->y);
  ...
  if (this->ENF().type == ENF::Aggressive || (this->parent && attacker)) {
    Character *closest = 0;
    unsigned char closest_distance =
        static_cast<int>(this->map->world->config["NPCChaseDistance"]);
    ...
    UTIL_FOREACH(this->map->characters, character) {
      ...
      int distance =
          util::path_length(character->x, character->y, this->x, this->y);
  ...
  if (attacker) {
    ...
    if ((absxdiff == 1 && absydiff == 0) || (absxdiff == 0 && absydiff == 1) ||
        (absxdiff == 0 && absydiff == 0)) {
    ...
    if (this->Walk(this->direction) == Map::WalkFail) {
      ...
      if (this->Walk(static_cast<Direction>(this->direction)) ==
          Map::WalkFail) {
...
void NPC::Talk() {
  ...
  UTIL_FOREACH(this->map->npcs, npc) {
    if (npc->id == this->id && npc->spawn_x == this->spawn_x &&
        npc->spawn_y == this->spawn_y) {
  ...
  if (util::rand(0.0, 1.0) <= data.talk_chance) {
    ...
    std::string phrase =
        data.talk_phrases[util::rand(0, data.talk_phrases.size() - 1)];
    talker->Say(phrase);
  }
}
...
bool NPC::Walk(Direction direction) { return this->map->Walk(this, direction); }
...
void NPC::Damage(Character *from, int amount, int spell_id) {
  ...
  if (this->hp > 0) {
    PacketBuilder builder(spell_id == -1 ? PACKET_NPC : PACKET_CAST,
                          PACKET_REPLY, 14);
    ...
    builder.AddShort(static_cast<short>(util::clamp<int>(
        static_cast<int>(double(this->hp) / double(this->ENF().hp) * 100.0), 0,
        100)));
...
void NPC::Killed(Character *from, int amount, int spell_id) {
  ...
  if (dropratemode == 1) {
    ...
    UTIL_FOREACH_CREF(this->Data().drops, checkdrop) {
      if (roll >= checkdrop->chance_offset &&
          roll < checkdrop->chance_offset + checkdrop->chance) {
        drop = checkdrop.get();
        break;
      }
    }
  ...
  if (drop) {
    dropid = drop->id;
    dropamount = std::min<int>(util::rand(drop->min, drop->max),
                               this->map->world->config["MaxItem"]);

    if (dropid <= 0 ||
        static_cast<std::size_t>(dropid) >=
            this->map->world->eif->data.size() ||
        dropamount <= 0)
      goto abort_drop;

    dropuid = this->map->GenerateItemID();

    std::shared_ptr<Map_Item> newitem(std::make_shared<Map_Item>(
        dropuid, dropid, dropamount, this->x, this->y, from->PlayerID(),
        Timer::GetTime() +
            static_cast<int>(this->map->world->config["ProtectNPCDrop"])));
    this->map->items.push_back(newitem);

    // Selects a random number between 0 and maxhp, and decides the winner based
    // on that
    switch (sharemode) {
    case 0:
      drop_winner = from;
...
abort_drop:
  ...
  UTIL_FOREACH(this->map->characters, character) {
    std::list<std::unique_ptr<NPC_Opponent>>::iterator findopp =
        this->damagelist.begin();
    for (;
         findopp != this->damagelist.end() && (*findopp)->attacker != character;
         ++findopp)
      ; // no loop body
    ...
    if (findopp != this->damagelist.end() || character->InRange(this)) {
      ...
      if (this->ENF().exp != 0) {
        if (findopp != this->damagelist.end()) {
          ...
          case 0:
            if (character == from) {
              ...
              if (reward > 0) {
                if (partysharemode) {
                  if (character->party) {
                    character->party->ShareEXP(reward, partysharemode,
                                               this->map);
                  } else {
          ...
          case 2:
            reward = int(std::ceil(
                double(this->ENF().exp) * exprate *
                (double((*findopp)->damage) / double(this->totaldamage))));
          ...
          case 3:
            reward = int(std::ceil(double(this->ENF().exp) * exprate *
                                   (double(this->damagelist.size()) / 1.0)));
          ...
          character->exp =
              std::min(character->exp,
                       static_cast<int>(this->map->world->config["MaxExp"]));

          while (character->level <
                     static_cast<int>(this->map->world->config["MaxLevel"]) &&
                 character->exp >=
                     this->map->world->exp_table[character->level + 1]) {
            level_up = true;
            ++character->level;
            character->statpoints +=
                static_cast<int>(this->map->world->config["StatPerLevel"]);
            character->skillpoints +=
                static_cast<int>(this->map->world->config["SkillPerLevel"]);
            character->CalculateStats();
          }

          if (level_up) {
            builder.SetID(spell_id == -1 ? PACKET_NPC : PACKET_CAST,
                          PACKET_ACCEPT);
      ...
      builder.AddShort(drop_winner ? drop_winner->PlayerID()
                                   : from->PlayerID());
      ...
      if ((sharemode == 0 && character == from) ||
          (sharemode != 0 && findopp != this->damagelist.end())) {
...
void NPC::Attack(Character *target) {
  int amount = util::rand(
      this->ENF().mindam,
      this->ENF().maxdam +
          static_cast<int>(this->map->world->config["NPCAdjustMaxDam"]));
  double rand = util::rand(0.0, 1.0);
  // Checks if target is facing you
  bool critical =
      std::abs(int(target->direction) - this->direction) != 2 ||
  ...
  formula_vars["modifier"] =
      1.0 / static_cast<double>(this->map->world->config["MobRate"]);
  formula_vars["damage"] = amount;
  formula_vars["critical"] = critical;

  amount = static_cast<int>(rpn_eval(
      rpn_parse(this->map->world->formulas_config["damage"]), formula_vars));
  double hit_rate = rpn_eval(
      rpn_parse(this->map->world->formulas_config["hit_rate"]), formula_vars);
  ...
  builder.AddThree(amount);
  builder.AddThree(util::clamp<int>(
      static_cast<int>(double(target->hp) / double(target->maxhp) * 100.0), 0,
      100));
  builder.AddByte(255);
  builder.AddByte(255);
...
void NPC::Say(const std::string &message) { this->map->Msg(this, message); }

#define v(x) vars[prefix + #x] = x;
#define vv(x, n) vars[prefix + n] = x;
#define vd(x) vars[prefix + #x] = data.x;

void NPC::FormulaVars(std::unordered_map<std::string, double> &vars,
                      std::string prefix) {
  const ENF_Data &data = this->ENF();
  vv(1, "npc") v(hp) vv(data.hp, "maxhp") vd(mindam) vd(maxdam) vd(accuracy)
      vd(evade) vd(armor) v(x) v(y) v(direction) vv(map->id, "mapid")
}
