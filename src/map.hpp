...
struct Map_Item {
  ...
  Map_Item(short uid_, short id_, int amount_, unsigned char x_,
           unsigned char y_, unsigned int owner_, double unprotecttime_)
      : uid(uid_), id(id_), amount(amount_), x(x_), y(y_), owner(owner_),
        unprotecttime(unprotecttime_) {}
};
...
struct Map_Warp {
  ...
  enum WarpSpec { NoDoor, Door, LockedSilver, LockedCrystal, LockedWraith };

  WarpSpec spec;
  bool open;

  Map_Warp()
      : map(0), x(0), y(0), levelreq(0), spec(Map_Warp::NoDoor), open(false) {}

  explicit operator bool() const { return map != 0; }
};
...
/**
 * Contains all information about a map, holds reference to contained Characters
 * and manages NPCs on it
 */
class Map {
private:
  bool Load();
...
public:
  enum WalkResult { WalkFail = 0, WalkOK = 1, WalkWarped = 2 };
  ...
  void SpellAttackPK(Character *from, Character *victim,
  ...
  std::shared_ptr<Map_Item> AddItem(short id, int amount, unsigned char x,
                                    unsigned char y, Character *from = 0);
  ...
  DelItem(std::list<std::shared_ptr<Map_Item>>::iterator it,
          Character *from = 0);
  ...
  std::vector<Character *> CharactersInRange(unsigned char x, unsigned char y,
                                             unsigned char range);
  std::vector<NPC *> NPCsInRange(unsigned char x, unsigned char y,
                                 unsigned char range);
  ...
  enum OccupiedTarget { PlayerOnly, NPCOnly, PlayerAndNPC };
  bool Occupied(unsigned char x, unsigned char y, Map::OccupiedTarget target,
                bool adminghost = false) const;
